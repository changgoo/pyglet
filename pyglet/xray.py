...
class Xray(object):
    def __init__(self, ytds, savdir='./', verbose=True,
                 emin=0.1, emax=10, nbins=1000,
                 redshift=0.00005, sky_center=(45.,30.),
                 model='apec', binscale='log'):
        # self.sim = sim
        self.ytds = ytds
    ...
    def create_profile(self,xy='z-vz'):
        ...
        if xy == 'z-vz':
            ...
            profile = yt.create_profile(
                ...
                fields=[("gas","volume"), ("gas", "mass"),
                        ("gas","xray_luminosity_0.5_2.0_keV"),("gas","xray_luminosity_0.5_7.0_keV")],
                n_bins=(256,384),
                units=dict(temperature="K", velocity_z="km/s", volume="pc**3", mass="Msun"),
                logs=dict(radius=False, velocity_z=False),
                weight_field=None,
                extrema=dict(temperature=(10,1.e10),velocity_z=(-1536, 1536)),
            )
    ...
    def set_filenames(self, axis='z', photon=True, event=True, simput=True):
        ...
        if photon: self.photon_fnames = photon_fnames
        if event: self.event_fnames = event_fnames
        if simput: self.simput_fnames = simput_fnames

    def make_photons(self,exp_time=(1000,"ks"), area=(1,"m**2"), redshift=0.00005,
      overwrite=False):
        if (not overwrite) and hasattr(self, 'photon_fnames'):
            if self.test_files(self.photon_fnames): return
    ...
    def project_photons(self,axis='z',nH=0.02,sky_center=(45.,30.),overwrite=False):
        if (not overwrite) and hasattr(self, 'event_fnames'):
            if self.test_files(self.event_fnames): return
    ...
    def test_files(self, fnames):
        ...
        if all_exist:
            if self.verbose: print("all files exist")
            return True
        else:
            return False
    ...
    def create_simput(self, axis='z', overwrite=False):
        ...
        if (not overwrite) and hasattr(self,'simput_fnames'):
            if self.test_files(self.simput_fnames): return
    ...
    def instrument_simulator(self, axis='z', targets=['src','onsrc','offsrc'],
                             exp=100, inst="lem_2eV", overwrite=False):
        if not hasattr(self,'simput_fnames'): self.create_simput(axis=axis)
        inst_fnames=dict()
        img_fnames=dict()
        spec_fnames=dict()
        for boxname, simput_fname in self.simput_fnames.items():
            for target in ['src','onsrc','offsrc']:
                if target == 'src':
                    ...
                    kwargs = dict(instr_bkgnd=False,
                                  foreground=False,
                                  ptsrc_bkgnd=False)
                elif target == 'onsrc':
                    sky_center = self.sky_center
                    kwargs = dict(instr_bkgnd=True,
                                  foreground=True,
                                  ptsrc_bkgnd=True)
                elif target == 'offsrc':
                    sky_center = (0,0)
                    kwargs = dict(instr_bkgnd=True,
                                  foreground=True,
                                  ptsrc_bkgnd=True)
                evt_fname = simput_fname.replace('simput','{}_{}ks_{}'.format(target,exp,inst))
    ...
    def parse_filename(self,f):
        ...
        inst = '_'.join(sp[6:-1])
        return dict(region=region, dist=dist, axis=axis,
                    src=src, exptime=exptime, inst=inst)
    ...
    def select_files(self,files,match=dict(exptime='100ks')):
        ...
        for f in files:
            ...
            for k,v in match.items():
                if v != par[k]: skip = True
            if skip: continue
            flist.append(f)

        return sorted(flist)

    def show(self, img_flist=None, spec_flist=None,
             match=dict(exptime='100ks'), width=0.2, vmin=0, vmax=100):
        ...
        for f in img_flist:
            ...
            self.plot_image(f,stretch='sqrt', cmap='arbre', width=width,
                            vmin=vmin, vmax=vmax,
                            fig=fig,grid_spec=gs[0,i], cbar=True)
    ...
    def clean_up(self, photon=False, event=False, simput=False,
                 inst=False, image=False, spec=False):
        if photon: self._remove('photon')
        if event: self._remove('event')
        if simput: self._remove('simput')
        if inst: self._remove('inst')
        if image: self._remove('img')
        if spec: self._remove('spec')

    def _remove(self, ftype):
        if hasattr(self, "{}_fnames".format(ftype)):
            for name,fname in getattr(self, "{}_fnames".format(ftype)).items():
                if os.path.isfile(fname):
                    if self.verbose: print("removing {}".format(fname))
                    os.remove(fname)
                if (ftype=='simput'):
